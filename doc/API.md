...
Now you can access TWA Meet API trough the `JitsiMeetJS` global object.

Components
=========

TWA Meet API has the following components:
...
*  `JitsiMeetJS.init(options)` - this method initialized TWA Meet API.
...
1. `JitsiConnection(appID, token, options)` - constructor. Creates the conference object.
    ...
    - `appID` - identification for the provider of TWA Meet video conferencing services. **NOTE: not implemented yet. You can safely pass `null`**
    - `token` - secret generated by the provider of TWA Meet video conferencing services. The token will be send to the provider from the TWA Meet server deployment for authorization of the current client.
...
1. The first thing you must do in order to use TWA Meet API is to initialize `JitsiMeetJS` object:
